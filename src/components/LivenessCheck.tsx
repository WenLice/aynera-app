...
import { completeLiveness, startLiveness } from "../api/liveness";
...
/**
 * Demo build: the static app's private front-camera face check. The capture stays on the device;
 * the demo stand-in records that the check was done so the flow moves on to photos.
 */
export function LivenessCheck({ open, onClose, onPassed }: Props) {
  ...
  const capture = async () => {
    ...
    try {
      ...
      await completeLiveness(session.sessionId);
      onPassed(session.sessionId);
      onClose();
    } catch {
      setHint("Couldn’t capture — try again");
      setBusy(false);
